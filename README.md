...
### 閲覧制限（プライバシー保護）
`/lookback` コマンドで表示されるログは、実行したユーザーがその瞬間にボイスチャンネルに参加していた（発言時の参加者リストに含まれている）場合のみ表示されます。これにより、自分が参加していない（聞いていない）会話の内容が漏洩することを防ぎます。

### ログの暗号化 (AES-256-GCM)
データベースに保存される会話ログ（`text`）は、AES-256-GCM方式で暗号化して保存することができます。
- **暗号化の有効化**: 環境変数 `ENCRYPTION_KEY` に 32バイト(256ビット) の16進数文字列（例: `openssl rand -hex 32` で生成した値）を設定してください。
- **未設定時の挙動**: `ENCRYPTION_KEY` が空または未設定の場合、**ログは暗号化されず平文のまま保存されます。** プライバシー保護のため、本番運用では必ず設定することを強く推奨します。

### ログの自動削除 (TTL)
MongoDBのTTLインデックスにより、会話ログは保存から **3日間** で自動的に削除されます。長期間のログ保持は行いません。
